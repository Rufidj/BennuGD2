                              ...
                              int clip_top, int clip_bottom)
{
    ...
    if (draw_top < draw_bot) {
        ...
        // Renderizar si tiene textura válida
        if (wall->texture > 0) {
                           ...
                           int screen_w, int screen_h)
{
    ...
    // Escanear regiones anidadas - ENCONTRAR TODAS LAS PAREDES QUE INTERSECTA EL RAYO
    // No solo la más cercana, sino TODAS
    for (int n = 0; n < opt_region->num_nested_regions; n++) {
        ...
        for (int w = 0; w < nested_opt->num_wall_ptrs; w++) {
            ...
            // CAMBIO CLAVE: Añadir TODAS las paredes que intersecta, no solo la más cercana
            if (t > 0.001f) {
                // Añadir esta pared a la lista de VDraws
                if (*num_vdraws >= MAX_VDRAWS) continue;
                
                // DEBUG: Mostrar cuando se añade una pared de región anidada
                static int debug_count = 0;
                if (debug_count < 30) {
                    printf("DEBUG: Col %d - Región #%d: type=%d, tex=%d, tex_top=%d, tex_bot=%d\n",
                           col, nested_idx, wall->type, wall->texture, wall->texture_top, wall->texture_bot);
                    debug_count++;
                }
                
                WLD_VDraw *vd = &vdraws[*num_vdraws];
                vd->wall = wall;
                vd->region_idx = nested_idx;
                vd->left_col = col;
                vd->distance = accumulated_distance + t;
                vd->clip_top = clip_top;
                vd->clip_bottom = clip_bottom;
                vd->adjacent_region = wall->back_region;
