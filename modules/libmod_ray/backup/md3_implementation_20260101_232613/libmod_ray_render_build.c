...
static void draw_sky_column(GRAPH *dest, int x, int y_start, int y_end) {
    ...
    for (int y = y_start; y <= y_end; y++) {
        ...
        if (g_zbuffer[i] < 100000.0f) continue;
        
        // Vertical Parallax (Pitch)
        // Standard "Horizon" adjustment
        // Map Horizon line to specific texture Y?
        // Simple scroll: tex_y = y + pitch
...
static void draw_plane_column(GRAPH *dest, int x, int y_start, int y_end, float height_diff, GRAPH *texture, int flags)
{
    ...
    for (int y = y_start; y <= y_end; y++) {
        ...
        if (g_engine.fogOn && !(flags & 1)) { // Don't fog hole punch lid (usually invisible or special)
             // Although if hole lid IS textured (grate), it should be fogged.
             // flags & 1 means "Hole Punch Z".
             // We allow drawing grate on top.
             float dist = z_depth; 
                         ...
                         if (!tex_upper) {
                            ...
                            for (int y = draw_top; y <= draw_bot; y++) {
                                 ...
                                 if (z_depth < g_zbuffer[pixel_idx]) {
                                     ...
                                     uint32_t pix = ray_sample_texture(tex_upper, tex_x, tex_y);
                            ...
                            float curr_v = (float)(draw_top - cy_top_curr) * v_step;
                             ...
                             for (int y = draw_top; y <= draw_bot; y++) {
                                 ...
                                 if (z_depth < g_zbuffer[pixel_idx]) {
                                     ...
                                     uint32_t pix = ray_sample_texture(tex_lower, tex_x, tex_y);
                                      if (pix == 0) pix = 0x00FF00; 
                                      if (g_engine.fogOn) pix = ray_fog_pixel(pix, z_depth * halfxdimen);
    ...
    if (sector->num_children > 0 && sector->child_sector_ids) {
        for (int i = 0; i < sector->num_children; i++) {
            ...
            if (child->is_solid) {
                 ...
                 // Non-solid child (Pit/Hole)
                 // First, render stencil to clear Z-buffer over the hole area
                 render_hole_stencil(dest, child_id, min_x, max_x);
                 // Then render the child interior
...
void ray_render_frame_build(GRAPH *dest)
{
    ...
    if (frame_debug++ % 120 == 0) {
        printf("DEBUG: SkyID=%d FPG=%d Width=%d Height=%d\n", g_engine.skyTextureID, g_engine.fpg_id, g_engine.displayWidth, g_engine.displayHeight);
