...
RAY_Model *ray_md2_load(const char *filename) {
    FILE *fp;
    RAY_Model *model;
    ...
    /* 1. Read Header */
    model = (RAY_Model *)malloc(sizeof(RAY_Model));
    if (!model) {
        fclose(fp);
        return NULL;
    }
...
void ray_md2_free(RAY_Model *model) {
    if (!model) return;
...
void ray_md2_interpolate_vertex(RAY_Model *model, int frame1, int frame2, float interpolation, int vertexIndex, vec3_t *out) {
