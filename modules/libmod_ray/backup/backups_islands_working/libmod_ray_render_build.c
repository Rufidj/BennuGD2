...
// NEAR_Z removed to avoid conflicts
...
static void draw_plane_column(GRAPH *dest, int x, int y_start, int y_end, float height_diff, GRAPH *texture)
{
    ...
    // Use half_h (vertical focal length) to match wall height projection formula:
    // y = half_h - (H * half_h) / z  <==>  z = (H * half_h) / (half_h - y)
    float view_dist = half_h; 
...
static void render_sector(GRAPH *dest, int sector_id, int min_x, int max_x, int depth)
{
    ...
    if (sector->num_children > 0 && sector->child_sector_ids) {
        for (int c = 0; c < sector->num_children; c++) {
            ...
            RAY_Sector *child_sector = &g_engine.sectors[child_id];
            
            // Render walls of child sector
            for (int w = 0; w < child_sector->num_walls; w++) {
                RAY_Wall *wall = &child_sector->walls[w];
                
                // Texture coordinates (Calculated early for use in swap)
                float u1 = 0, u2 = 0;
                float dist_sq = powf(wall->x2 - wall->x1, 2) + powf(wall->y2 - wall->y1, 2);
                float dist = sqrtf(dist_sq);
                u2 = dist;
                ...
                // Project to screen
                int32_t sx1, sy1, sx2, sy2;
                int orientation = get_screen_coords(p1, p2, &sx1, &sy1, &sx2, &sy2);
                if (!orientation) {
                    continue;
                }
                ...
                // --------------------------------------------------------------------
                // ISLAND RENDER LOGIC: Standard Build Recursion
                // --------------------------------------------------------------------
                // We draw the outer walls of the island (Riser/Soffit).
                // Then we recurse into the island sector through the "portal" formed by its boundary.
                // The recursion handles drawing the Lid (floor of the island) because from the "inside"
                // perspective, the back walls of the island will draw their floor planes towards us.

                float z1 = (float)sy1;
                float z2 = (float)sy2;
                
                // Texture coordinates (Moved up)
                // float u1 = 0, u2 = 0;
                // ... already calculated above

                // 1. Calculate Heights
                // Parent (Current) Heights
                float p_floor_h = sector->floor_z - g_engine.camera.z;
                float p_ceil_h = sector->ceiling_z - g_engine.camera.z;
                // Child (Island) Heights
                float c_floor_h = child_sector->floor_z - g_engine.camera.z;
                float c_ceil_h = child_sector->ceiling_z - g_engine.camera.z;
                
                // Screen Y coordinates
                // Parent Window (what we usually see)
                int py1_top = halfydimen - (int)((p_ceil_h * halfydimen) / z1);
                int py1_bot = halfydimen - (int)((p_floor_h * halfydimen) / z1);
                int py2_top = halfydimen - (int)((p_ceil_h * halfydimen) / z2);
                int py2_bot = halfydimen - (int)((p_floor_h * halfydimen) / z2);
                
                // Child Window (The aperture of the island block)
                int cy1_top = halfydimen - (int)((c_ceil_h * halfydimen) / z1);
                int cy1_bot = halfydimen - (int)((c_floor_h * halfydimen) / z1);
                int cy2_top = halfydimen - (int)((c_ceil_h * halfydimen) / z2);
                int cy2_bot = halfydimen - (int)((c_floor_h * halfydimen) / z2);
                
                // 2. Render RISER (Lower Step)
                // Texture coordinates must be swapped if BackFace because u1/u2 are tied to P1/P2,
                // but screen points sx1/sx2 have been swapped.
                // Heights (py1, etc) are calculated from z1/z2 which ARE already swapped, so they match sx1/sx2.
                if (orientation == -1) {
                    float ftmp = u1; u1 = u2; u2 = ftmp;
                }

                // Range: [Child Floor, Parent Floor] (e.g. 100 to 200)
                // Visual Top = cy_bot, Visual Bot = py_bot.
                int r_y1_top = (cy1_bot < py1_bot) ? cy1_bot : py1_bot;
                int r_y1_bot = (cy1_bot > py1_bot) ? cy1_bot : py1_bot;
                
                int r_r_y2_top = (cy2_bot < py2_bot) ? cy2_bot : py2_bot;
                int r_r_y2_bot = (cy2_bot > py2_bot) ? cy2_bot : py2_bot;
                
                GRAPH *tex_lower = NULL;
                if (wall->texture_id_middle > 0) tex_lower = bitmap_get(g_engine.fpg_id, wall->texture_id_middle);
                else if (wall->texture_id_lower > 0) tex_lower = bitmap_get(g_engine.fpg_id, wall->texture_id_lower);
                
                // Render Riser
                // Use a small Z-bias to prevent Z-fighting with the parent floor
                draw_wall_segment_linear(dest, sx1, sx2,
                                         r_y1_top, r_r_y2_top,
                                         r_y1_bot, r_r_y2_bot,
                                         z1 - 0.5f, z2 - 0.5f, tex_lower, u1, u2, sector,
                                         min_x, max_x, 1); // flag 1 = WALL ONLY
                
                // 3. Render SOFFIT (Upper Step)
                // Range: [Parent Ceil, Child Ceil]
                int l_y1_top = (py1_top < cy1_top) ? py1_top : cy1_top;
                int l_y1_bot = (py1_top > cy1_top) ? py1_top : cy1_top;
                int l_y2_top = (py2_top < cy2_top) ? py2_top : cy2_top;
                int l_y2_bot = (py2_top > cy2_top) ? py2_top : cy2_top;

                GRAPH *tex_upper = NULL;
                if (wall->texture_id_upper > 0) tex_upper = bitmap_get(g_engine.fpg_id, wall->texture_id_upper);
                
                if (tex_upper) {
                    draw_wall_segment_linear(dest, sx1, sx2,
                                             l_y1_top, l_y2_top,
                                             l_y1_bot, l_y2_bot,
                                             z1 - 0.5f, z2 - 0.5f, tex_upper, u1, u2, sector,
                                             min_x, max_x, 1);
                }

                // 4. RECURSION (Render Lid/Inside)
                // We assume the "window" into the island is the gap between its own Ceiling and Floor.
                // Anything strictly INSIDE that vertical range is part of the island's volume visible to us.
                // The Lid (Floor) will be drawn by the back walls of the island rendering their floor plane.
                
                // Save clipping
                int16_t saved_umost[MAXSCREENWIDTH];
                int16_t saved_dmost[MAXSCREENWIDTH];
                
                // Interpolate Portal Window [cy_top, cy_bot]
                float dx = (float)(draw_x2 - draw_x1);
                float span = (float)(sx2 - sx1);
                if (span < 1.0f) span = 1.0f;
                
                float d_win_top = (float)(cy2_top - cy1_top) / span;
                float d_win_bot = (float)(cy2_bot - cy1_bot) / span;
                
                float win_top = (float)cy1_top + d_win_top * (float)(draw_x1 - sx1);
                float win_bot = (float)cy1_bot + d_win_bot * (float)(draw_x1 - sx1);
                
                for (int x = draw_x1; x <= draw_x2; x++) {
                    saved_umost[x] = umost[x];
                    saved_dmost[x] = dmost[x];
                    
                    int new_top = (int)win_top;
                    int new_bot = (int)win_bot;
                    
                    // Constrain to current view window
                    if (new_top < umost[x]) new_top = umost[x];
                    
                    // CRITICAL FIX FOR ISLAND LIDS:
                    // Usually for a portal (hole), we clip dmost to new_bot.
                    // But for an ISLAND (Platform), we want to see the LID (Child Floor).
                    // This Lid extends from 'new_bot' (the edge we see) DOWNWARDS towards the back of the island.
                    // If we clip dmost to new_bot, we restrict drawing to ABOVE the edge, preventing the Lid from appearing.
                    // So we must NOT restrict dmost for islands, trusting z-buffer or painter's algo to handle overlap?
                    // Wait. If we don't restriction dmost, we might draw over the Riser?
                    // No, the Riser was already drawn.
                    // The Lid is drawn by child sector floor logic.
                    // We need to allow it to draw down to the original dmost.
                    
                    // if (new_bot > dmost[x]) new_bot = dmost[x]; 
                    // dmost[x] = (int16_t)new_bot;
                    
                    // We essentially only clip the TOP (Ceiling/Soffit).
                    // The bottom is open for the Lid to sit on top of the platform.
                    // But wait, if we are inside the child, 'dmost' is the limit of the floor.
                    // If we leave dmost as parent's dmost, the child floor will draw all the way down to parent's floor occlusion?
                    // Yes, that's what we want! (Until it hits the back wall of the island).
                    
                    umost[x] = (int16_t)new_top;
                    // dmost[x] = dmost[x]; // Unchanged
                ...
                // Recurse
                render_sector(dest, child_id, draw_x1, draw_x2, depth + 1);
