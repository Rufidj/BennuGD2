...
begin
    ...
    ent_id.graph = 32;
    ent_id.x = 416.356; ent_id.y = 303.916; ent_id.z = 0;
    ent_id.angle = 0; ent_id.size = 100;

    // Text Entity: Label
    write(0, 365, 199, 0, "Hola Mundo");
    // Text Entity: btn_exit
    Auto_Btn_btn_exit_2356();
    // Text Entity: btn_menu_2
    Auto_Btn_btn_menu_2_4953();
...
process Auto_Btn_btn_exit_2356()
PRIVATE
    int txt_id;
    int w, h;
begin
    x = 374; y = 401;
    txt_id = write(0, x, y, 0, "[ SALIR ]");
    w = text_width(0, "[ SALIR ]");
    h = text_height(0, "[ SALIR ]");
    loop
...
process Auto_Btn_btn_menu_2_4953()
PRIVATE
    int txt_id;
    int w, h;
begin
    x = 369; y = 300;
    txt_id = write(0, x, y, 0, "IR MENU 2");
    w = text_width(0, "IR MENU 2");
    h = text_height(0, "IR MENU 2");
    loop
        if (mouse.left AND (mouse.x > x AND mouse.x < (x + w)) AND abs(mouse.y - y) < (h/2 + 5))
             begin
                 goto_scene("menu_2");
